		...
		<div className="space-y-10">
			<header className="site-page-header space-y-5">
				<div className="space-y-2">
					<p className="text-xs font-medium uppercase tracking-[0.28em] text-[var(--site-fg-muted)]">
		...
		<div className="space-y-8">
			<header className="site-page-header space-y-2">
				...
				<p className="site-page-lead">
					No {kind} named <span className="font-mono">{value}</span> exists in
					the current photo archive.
				</p>
			</header>
		...
		<nav className="flex flex-wrap gap-2" aria-label="Photo collections">
			{items.map((item) => (
				<Link
					key={item.id}
					to={item.href}
					className={`site-tag transition ${
						item.id === currentId ? "text-[var(--site-fg)]" : ""
					}`}
				>
					{item.title}
