...
import { getGridUnit as layoutGridUnit, type GalleryFrameRect } from '~/lib/galleryLayout'
import { getFrameSplashHandoffWorldRect, getFrameWorldRect } from '~/lib/galleryLayoutStore'
...
export function thumbLayoutForIndex(images: CateImage[], index: number, scrollOffset: number) {
  const grid = getGridUnit()
...
export function scrollOffsetForThumbCenter(images: CateImage[], index: number) {
  const { y } = thumbLayoutForIndex(images, index, 0)
  return -y
}

export function nearestThumbIndex(images: CateImage[], scrollOffset: number) {
  let best = 0
